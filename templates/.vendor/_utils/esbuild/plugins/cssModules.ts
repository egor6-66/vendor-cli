...
async function buildScss(scssFullPath: string, sassOptions: sass.Options<any>): Promise<any> {
            ...
            build.onResolve({ filter: /\.modules?\.scss$/, namespace: 'file' }, async (args) => {
                ...
                const result = await (async () => {
                    ...
                    if (outdir) {
                        ...
                        const targetSubpath =
                            absoluteOutdir.indexOf(entryRelDir) === -1 ? path.join(entryRelDir, `${sourceBaseName}.css.js`) : `${sourceBaseName}.css.js`;
